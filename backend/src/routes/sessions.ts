...
import type { SessionDocument, SessionBadge } from '../../../common/src/session.js';
...
function validateSessionPayload(body: any): { valid: boolean; errors?: string[] } {
  const errors: string[] = [];
  if (!body) {
  ...
  if (!Array.isArray(body.blockedSites)) errors.push('blockedSites must be an array');
  if (!body.startDate || !isISODateString(body.startDate)) errors.push('startDate must be an ISO date string');
  if (!body.status || typeof body.status !== 'string') errors.push('status is required');
  if (!body.device || typeof body.device !== 'object' || !body.device.deviceId) errors.push('device.deviceId is required');
  if (typeof body.existsLocally !== 'boolean') errors.push('existsLocally must be boolean');
...
router.post('/', requireAuth, async (req: Request, res: Response) => {
  ...
  if (!valid) return res.status(400).json({ error: 'Invalid payload', details: errors });
...
router.post('/:id/ping', requireAuth, async (req: Request, res: Response) => {
  ...
  await sessions.updateOne({ id: sessionId }, { $set: { lastCheckedAt: now } });
...
router.patch('/:id/status', requireAuth, async (req: Request, res: Response) => {
  ...
  const allowed = ['stained', 'removed', 'active', 'completed', 'scheduled'];
  ...
  if (status === 'removed' || status === 'completed') update.endDate = new Date().toISOString();
...
// GET /sessions/current-badge - get current session badge for owner
router.get('/current-badge', requireAuth, async (req: Request, res: Response) => {
  const auth = (req as any).auth;
  const ownerId = auth.userId as string;
  ...
  const sessions = db.collection('sessions');
  // find the most relevant active/scheduled session
  const doc = await sessions.find({ ownerId, status: { $in: ['active', 'scheduled'] } }).sort({ startDate: -1 }).limit(1).toArray();
  if (!doc || doc.length === 0) return res.status(404).json({ error: 'No active session' });
  const s = doc[0] as unknown as SessionDocument;
